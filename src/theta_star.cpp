...
bool ThetaStar::Plan(const Mat& cost_map, const Point& start_point, const Point& end_point, std::vector<Point>& path, PlannerAction planner_action) {
  cost_map_ = cost_map.clone();

  src_ = {static_cast<int>(start_point.x), static_cast<int>(start_point.y)};
  dst_ = {static_cast<int>(end_point.x), static_cast<int>(end_point.y)};
  ...
  while (!queue_.empty()) {
    ...
    close_list_.emplace_back(curr_data->x, curr_data->y);
...
void ThetaStar::backtrace(std::vector<Point>& raw_points, const tree_node* curr_n) const {
...
#if 1
  for (auto& i : path_rev) {
    raw_points.emplace_back(i.x, i.y);
  }
#else
  for (int i = 0; i < path_rev.size() - 1; i++) {
    auto points = InsertPoints(cost_map_, {path_rev[i].x, path_rev[i].y}, {path_rev[i + 1].x, path_rev[i + 1].y});
    raw_points.insert(raw_points.end(), points.begin(), points.end());
  }
#endif
...
void ThetaStar::resetContainers() {
  ...
  int curr_size_x = static_cast<int>(cost_map_.cols);
  int curr_size_y = static_cast<int>(cost_map_.rows);
