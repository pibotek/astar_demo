...
class ThetaStar : public Planner {
 public:
  coordsM src_{}, dst_{};
  ...
  inline bool isSafe(const int& cx, const int& cy) const {
    return cost_map_.at<unsigned char>(cy, cx) < LETHAL_COST;
  }
 ...
 protected:
   ...
   */
  void backtrace(std::vector<Point>& raw_points, const tree_node* curr_n) const;
   ...
   */
  bool isSafe(const int& cx, const int& cy, double& cost) {
   ...
   */
  inline double getCost(const int& cx, const int& cy) {
    return 26 + 0.9 * cost_map_.at<unsigned char>(cy, cx);
  }
